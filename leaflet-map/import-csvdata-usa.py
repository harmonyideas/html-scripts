...
file_local = '/usr/local/bin/www-scripts/usacounties.json'
file_www = '/var/www/html/harmonyideas.com/map/usacounties.json'
with open(file_local, 'r') as f:
    data = json.load(f)

for updaterow in covid19_df.itertuples():
    for row in data['features']:
        if (float(row['properties']['GEOID']) == updaterow.FIPS):
